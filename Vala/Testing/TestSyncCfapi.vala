...
const int CFVERIFY_VIRTUAL (folder, path)
    ...
    QVERIFY (cfapi.isSparseFile ( (folder).local_path () + (path)));
...
class TestSyncCfApi : GLib.Object {
    ...
    ***********************************************************/
    private void on_signal_test_virtual_file_lifecycle () {
        ...
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a1").size (), 64);
        ...
        QVERIFY (fakeFolder.sync_once ());
        CFVERIFY_VIRTUAL (fakeFolder, "A/a1");
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a1").size (), 65);
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a1").lastModified (), someDate);
        QVERIFY (fakeFolder.current_remote_state ().find ("A/a1"));
        ...
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a1m").size (), 65);
        ...
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a2").size (), 32);
        CFVERIFY_VIRTUAL (fakeFolder, "A/a3");
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a3").size (), 33);
        on_signal_cleanup ();
    ...
    ***********************************************************/
    private void on_signal_test_virtual_file_conflict () {
        ...
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a1").size (), 11);
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/a2").size (), 12);
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "B/b1").size (), 21);
        on_signal_cleanup ();
    ...
    ***********************************************************/
    private void on_signal_test_with_normal_sync () {
        ...
        QCOMPARE (GLib.FileInfo (fakeFolder.local_path () + "A/new").size (), 42);
    ...
    ***********************************************************/
    private void on_signal_test_virtual_file_download_resume () {
        ...
        QVERIFY (!fakeFolder.sync_once ());
        QVERIFY (itemInstruction (completeSpy, "A/a1", CSYNC_INSTRUCTION_SYNC));
        QVERIFY (cfapi.isSparseFile (fakeFolder.local_path () + "A/a1"));
        QVERIFY (GLib.FileInfo (fakeFolder.local_path () + "A/a1").exists ());
    ...
    ***********************************************************/
    private void on_signal_test_new_files_not_virtual () {
        ...
        setPinState (fakeFolder.local_path (), PinState.PinState.ALWAYS_LOCAL, cfapi.NoRecurse);
    ...
    ***********************************************************/
    ...
    private void on_signal_test_sync_dehydration () {
        ...
        var isDehydrated = [&] (string path) {
            return cfapi.isSparseFile (fakeFolder.local_path () + path)
    ...
    ***********************************************************/
    private void on_signal_test_new_virtuals () {
        ...
        setPinState (fakeFolder.local_path () + "local", PinState.PinState.ALWAYS_LOCAL, cfapi.Recurse);
        setPinState (fakeFolder.local_path () + "online", PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.Recurse);
        setPinState (fakeFolder.local_path () + "unspec", PinState.PinState.UNSPECIFIED, cfapi.Recurse);
        ...
        setPinState (fakeFolder.local_path (), PinState.PinState.ALWAYS_LOCAL, cfapi.Recurse);
        ...
        setPinState (fakeFolder.local_path (), PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.Recurse);
        // Need to force pin state for the subfolders again
        setPinState (fakeFolder.local_path () + "local", PinState.PinState.ALWAYS_LOCAL, cfapi.Recurse);
        setPinState (fakeFolder.local_path () + "online", PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.Recurse);
        setPinState (fakeFolder.local_path () + "unspec", PinState.PinState.UNSPECIFIED, cfapi.Recurse);
    ...
    ***********************************************************/
    private void on_signal_test_availability () {
        ...
        setPinState (fakeFolder.local_path () + "local/sub", PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.NoRecurse);
        QCOMPARE (*vfs.availability ("local"), VfsItemAvailability.VfsItemAvailability.ALL_HYDRATED);
        setPinState (fakeFolder.local_path () + "online/sub", PinState.PinState.UNSPECIFIED, cfapi.NoRecurse);
        ...
        setPinState (fakeFolder.local_path () + "local/file2", PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.NoRecurse);
        setPinState (fakeFolder.local_path () + "online/file2", PinState.PinState.ALWAYS_LOCAL, cfapi.NoRecurse);
        QVERIFY (fakeFolder.sync_once ());
    ...
    ***********************************************************/
    private void on_signal_test_pin_state_locals () {
        ...
        setPinState (fakeFolder.local_path () + "local", PinState.PinState.ALWAYS_LOCAL, cfapi.NoRecurse);
        setPinState (fakeFolder.local_path () + "online", PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.NoRecurse);
        setPinState (fakeFolder.local_path () + "unspec", PinState.PinState.UNSPECIFIED, cfapi.NoRecurse);
    ...
    ***********************************************************/
    private on_ void testEmptyFolderInOnlineOnlyRoot () {
        ...
        setPinState (fakeFolder.local_path (), PinState.VfsItemAvailability.ONLINE_ONLY, cfapi.NoRecurse);
    ...
    ***********************************************************/
    private on_ void testOpeningOnlineFileTriggersDownload () {
        ...
        fakeFolder.remote_modifier ().insert ("online/sub/file1", 10 * 1024 * 1024);
        QVERIFY (fakeFolder.sync_once ());
        ...
        if (errorKind == Timeout) {
            ...
            fakeFolder.server_error_paths ().append ("online/sub/file1", errorKind);
        }
        ...
        std.thread t ([&] {
            GLib.File file = GLib.File.new_for_path (fakeFolder.local_path () + "online/sub/file1");
