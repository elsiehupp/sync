...
public class TestSelectiveSync : GLib.Object {
    ...
    ***********************************************************/
    private void test_selective_sync_big_folders () {
        ...
        QSignalSpy new_big_folder = new QSignalSpy (fake_folder.sync_engine (), SyncEngine.new_big_folder);
        ...
        GLib.assert_true (new_big_folder.count () == 1);
        GLib.assert_true (new_big_folder.first ()[0].to_string () == "A/new_big_dir");
        GLib.assert_true (new_big_folder.first ()[1].to_bool () == false);
        ...
        GLib.assert_true (new_big_folder.count () == 1); // (since we don't have a real Folder, the files were not added to any list)
        new_big_folder.clear ();
