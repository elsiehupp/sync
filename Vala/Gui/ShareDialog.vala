    ...
    ***********************************************************/
    ...
    Share_dialog.Share_dialog (QPointer<AccountState> account_state,
        ...
        if (sharing_possible) {
            ...
            connect (this.manager, &Share_manager.on_shares_fetched, this, &Share_dialog.on_shares_fetched);
            connect (this.manager, &Share_manager.on_link_share_created, this, &Share_dialog.on_add_link_share_widget);
            connect (this.manager, &Share_manager.on_link_share_requires_password, this, &Share_dialog.on_link_share_requires_password);
        }
    }
    ...
    Share_link_widget *Share_dialog.add_link_share_widget (unowned<Link_share> link_share) {
        ...
        if (this.manager) {
            connect (this.manager, &Share_manager.on_server_error, link_share_widget, &Share_link_widget.on_server_error);
        }
        ...
        //connect (this.link_widget_list.at (index), &Share_link_widget.resize_requested, this, &Share_dialog.on_adjust_scroll_widget_size);
    ...
    void Share_dialog.init_link_share_widget () {
        if (this.link_widget_list.size () == 0) {
            ...
            connect (this.empty_share_link_widget, &Share_link_widget.resize_requested, this, &Share_dialog.on_adjust_scroll_widget_size);
            connect (this, &Share_dialog.toggle_share_link_animation, this.empty_share_link_widget, &Share_link_widget.on_toggle_share_link_animation);
            connect (this.empty_share_link_widget, &Share_link_widget.create_link_share, this, &Share_dialog.on_create_link_share);

            connect (this.empty_share_link_widget, &Share_link_widget.create_password, this, &Share_dialog.on_create_password_for_link_share);
    ...
    void Share_dialog.show_sharing_ui () {
        ...
        if (user_group_sharing) {
            ...
            // Connect style_changed events to our widget, so it can adapt (Dark-/Light-Mode switching)
            connect (this, &Share_dialog.style_changed, this.user_group_widget, &Share_user_group_widget.on_style_changed);

            this.ui.vertical_layout.insert_widget (1, this.user_group_widget);
            this.user_group_widget.on_get_shares ();
        }
