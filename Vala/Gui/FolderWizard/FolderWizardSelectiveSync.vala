    ...
    ***********************************************************/
    public FolderWizardSelectiveSync (Account account) {
        ...
        if (Theme.show_virtual_files_option && this.best_available_vfs_mode != Vfs.Off) {
            this.virtual_files_check_box = new QCheckBox (_("Use virtual files instead of downloading content immediately %1").printf (this.best_available_vfs_mode == Vfs.WindowsCfApi ? "" : _(" (experimental)")));
            ...
            this.virtual_files_check_box.checked (this.best_available_vfs_mode == Vfs.WindowsCfApi);
    ...
    public override void initialize_page () {
        ...
        if (this.virtual_files_check_box) {
            ...
            if (Utility.is_path_windows_drive_partition_root (wizard ().field ("source_folder").to_string ())) {
                ...
                this.virtual_files_check_box.on_signal_text (_("Use virtual files instead of downloading content immediately %1").printf (this.best_available_vfs_mode == Vfs.WindowsCfApi ? "" : _(" (experimental)")));
