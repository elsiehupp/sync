...
class SocketApi : GLib.Object {
    ...
    ***********************************************************/
    ...
    private SocketApiServer this.local_server;
    ...
    ***********************************************************/
    ...
    /***********************************************************
    ***********************************************************/
    public void on_signal_update_folder_view (Folder f) {
    ...
    ***********************************************************/
    private void on_signal_read_socket () {
        ...
        static var invalid_listener = unowned SocketListener.create (null);
        ...
        while (socket.can_read_line ()) {
            ...
            if (command.starts_with ("ASYNC_")) {
                ...
                if (index_of_method != -1) {
                    static_meta_object.method (index_of_method)
                        .invoke (this, Qt.QueuedConnection,
                            Q_ARG (unowned SocketApiJob, socket_api_job));
                } else {
                ...
                var socket_api_job = unowned SocketApiJobV2.create (listener, command, json);
                if (index_of_method != -1) {
                    static_meta_object.method (index_of_method)
                        .invoke (this, Qt.QueuedConnection,
                            Q_ARG (unowned SocketApiJobV2, socket_api_job));
                } else {
                    GLib.warning ("The command is not supported by this version of the client:" + command
                                        + "with argument:" + argument;
    ...
    ***********************************************************/
    private struct FileData {
        ...
        ***********************************************************/
        ...
        /***********************************************************
        ***********************************************************/
        static FileData get_file_data (string local_file) {
            FileData data;
        ...
        ***********************************************************/
        FileData parent_folder () {
            return FileData.get_file_data (GLib.FileInfo (local_path).directory ().path ().to_utf8 ());
        }
    ...
    ***********************************************************/
    private void process_share_request (string local_file, SocketListener listener, ShareDialogStartPage start_page) {
        ...
        var file_data = FileData.get_file_data (local_file);
    ...
    ***********************************************************/
    private void process_file_activity_request (string local_file) {
        const var file_data = FileData.get_file_data (local_file);
    ...
    ***********************************************************/
    private void command_RETRIEVE_FILE_STATUS (string argument, SocketListener listener) {
        ...
        var file_data = FileData.get_file_data (argument);
        if (!file_data.folder) {
    ...
    ***********************************************************/
    private void command_MAKE_AVAILABLE_LOCALLY (string files_arg, SocketListener listener) {
        ...
        for (var file : files) {
            var data = FileData.get_file_data (file);
            if (!data.folder)
                continue;
    ...
    ***********************************************************/
    ...
    External sync
    ***********************************************************/
    private void command_V2_LIST_ACCOUNTS (unowned SocketApiJobV2 job) {
        QJsonArray out;
    ...
    External sync
    ***********************************************************/
    private void command_V2_UPLOAD_FILES_FROM (unowned SocketApiJobV2 job) {
        var upload_job = new SocketUploadJob (job);
    ...
    ***********************************************************/
    private void command_GET_MENU_ITEMS (string argument, SocketListener listener) {
        ...
        if (sync_folder && files.size () == 1 && sync_folder.account_state ().is_connected ()) {
            ...
            FileData file_data = FileData.get_file_data (argument);
        ...
        if (sync_folder
            ...
            for (var file : files) {
                var file_data = FileData.get_file_data (file);
    ...
    ***********************************************************/
    ...
    Direct Editing
    ***********************************************************/
    private DirectEditor get_direct_editor_for_local_file (string local_file) {
        FileData file_data = FileData.get_file_data (local_file);
        var capabilities = file_data.folder.account_state ().account ().capabilities ();
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_ASSERT_ICON_IS_EQUAL (unowned SocketApiJob job) {
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_LIST_WIDGETS (unowned SocketApiJob job) {
        string response;
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_INVOKE_WIDGET_METHOD (unowned SocketApiJob job) {
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_GET_WIDGET_PROPERTY (unowned SocketApiJob job) {
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_SET_WIDGET_PROPERTY (unowned SocketApiJob job) {
        var arguments = job.arguments ();
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_WAIT_FOR_WIDGET_SIGNAL (unowned SocketApiJob job) {
        var arguments = job.arguments ();
        string widget_name = arguments["object_name"].to_string ();
        var widget = find_widget (arguments["object_name"].to_string ());
        if (!widget) {
        ...
        ListenerClosure closure = new ListenerClosure ([job] () {
        ...
        connect (widget, signal_signature_final, closure, SLOT (on_signal_closure ()), Qt.QueuedConnection);
    }
    ...
    #if GUI_TESTING
    ***********************************************************/
    private void command_ASYNC_TRIGGER_MENU_ACTION (unowned SocketApiJob job) {
        var arguments = job.arguments ();
