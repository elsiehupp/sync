...
namespace {
    ...
    QVector<char> get_illegal_chars_from_string (string string) {
        QVector<char> result;
    ...
    string illegal_character_list_to_string (QVector<char> &illegal_characters) {
...
class Invalid_filename_dialog : Gtk.Dialog {
    ...
    ***********************************************************/
    ...
    Invalid_filename_dialog.Invalid_filename_dialog (AccountPointer account, Folder folder, string file_path, Gtk.Widget parent)
        ...
        const var file_path_file_info = QFileInfo (_file_path);
        ...
        _ui.button_box.button (QDialogButtonBox.Ok).on_set_text (_("Rename file"));

        _ui.description_label.on_set_text (_("The file %1 could not be synced because the name contains characters which are not allowed on this system.").arg (_original_file_name));
        _ui.explanation_label.on_set_text (_("The following characters are not allowed on the system : * \" | & ? , ; : \\ / ~ < >"));
    ...
    void Invalid_filename_dialog.check_if_allowed_to_rename () {
        const var propfind_job = new PropfindJob (_account, QDir.clean_path (_folder.remote_path () + _original_file_name));
    ...
    void Invalid_filename_dialog.on_accept () {
        ...
        const var propfind_job = new PropfindJob (_account, QDir.clean_path (_folder.remote_path () + _new_filename));
    ...
    void Invalid_filename_dialog.on_filename_line_edit_text_changed (string text) {
        const var is_new_file_name_different = text != _original_file_name;
        ...
        const var is_text_valid = is_new_file_name_different && !contains_illegal_chars;

        if (is_text_valid) {
            _ui.error_label.on_set_text ("");
        } else {
            _ui.error_label.on_set_text (_("Filename contains illegal characters : %1")
    ...
    void Invalid_filename_dialog.on_move_job_finished () {
        ...
        if (error != QNetworkReply.NoError) {
            _ui.error_label.on_set_text (_("Could not rename file. Please make sure you are connected to the server."));
            return;
        }
    ...
    void Invalid_filename_dialog.on_remote_file_already_exists (QVariantMap &values) {
        ...
        _ui.error_label.on_set_text (_("Cannot rename file because a file with the same name does already exist on the server. Please pick another name."));
        _ui.button_box.button (QDialogButtonBox.Ok).set_enabled (false);
    }
    ...
    void Invalid_filename_dialog.on_remote_file_does_not_exist (QNetworkReply reply) {
        ...
        const var remote_source = QDir.clean_path (_folder.remote_path () + _original_file_name);
        const var remote_destionation = QDir.clean_path (_account.dav_url ().path () + _folder.remote_path () + _new_filename);
        const var move_job = new Move_job (_account, remote_source, remote_destionation, this);
