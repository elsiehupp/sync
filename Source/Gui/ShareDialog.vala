...
private slots:
    ...
    void slotCreatePasswordForLinkShare (string &password);
...
private:
    ...
    ShareDialog.ShareDialog (QPointer<AccountState> accountState,
        ...
        if (ocDir.isEmpty ()) {
            ...
            _ui.label_sharePath.setText (string ());
        } else {
    ...
    void ShareDialog.slotSharesFetched (QList<QSharedPointer<Share>> &shares) {
        ...
        const string versionString = _accountState.account ().serverVersion ();
    ...
    void ShareDialog.slotCreateLinkShare () {
        if (_manager) {
            ...
            const auto password = askOptionalPassword ? createRandomPassword () : string ();
            _manager.createLinkShare (_sharePath, string (), password);
        }
    }
    
    void ShareDialog.slotCreatePasswordForLinkShare (string &password) {
    ...
    void ShareDialog.slotLinkShareRequiresPassword () {
        bool ok = false;
        string password = QInputDialog.getText (this,
