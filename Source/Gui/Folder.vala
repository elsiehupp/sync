...
public:
    ...
    ***********************************************************/
    string alias ();
    string shortGuiRemotePathOrAppName (); // since 2.0 we don't want to show aliases anymore, show the path instead

    /***********************************************************
    short local path to display on the GUI  (native separators)
    ***********************************************************/
    string shortGuiLocalPath ();
    ...
    ***********************************************************/
    string remotePathTrailingSlash ();
    ...
    ***********************************************************/
    ...
    /***********************************************************
      ...
      */
    bool isFileExcludedAbsolute (string &fullPath) const;

    /***********************************************************
      * Returns whether a file inside this folder should be excluded.
      */
    bool isFileExcludedRelative (string &relativePath) const;
    ...
    ***********************************************************/
    ...
    string fileFromLocalPath (string &localPath) const;
...
signals:
    ...
    void newBigFolderDiscovered (string &); // A new folder bigger than the threshold was discovered
    ...
    by sync activity.
    ***********************************************************/
    void watchedFileChangedExternally (string &path);
...
public slots:
    ...
    /***********************************************************
       ...
       */
    void slotWatchedPathChanged (string &path, ChangeReason reason);
    ...
    ***********************************************************/
    ...
    discovery.
    ***********************************************************/
    void schedulePathForLocalDiscovery (string &relativePath);
...
private slots:
    ...
    ***********************************************************/
    void slotSyncError (string &message, ErrorCategory category = ErrorCategory.Normal);

    void slotAddErrorToGui (SyncFileItem.Status status, string &errorMessage, string &subject = {});
    ...
    void slotNewBigFolderDiscovered (string &, bool isExternal);
    ...
    ***********************************************************/
    void slotFolderConflicts (string &folder, QStringList &conflictPaths);
    ...
    void slotWatcherUnreliable (string &message);
...
private:
    ...
    void createGuiLog (string &filename, LogStatus status, int count,
        const string &renameTarget = string ());
    ...
    string _canonicalLocalPath; // As returned with QFileInfo:canonicalFilePath.  Always ends with "/"
...
string Folder.shortGuiRemotePathOrAppName () {
    if (remotePath ().length () > 0 && remotePath () != QLatin1String ("/")) {
        string a = QFile (remotePath ()).fileName ();
        if (a.startsWith ('/')) {
            a = a.remove (0, 1);
        }
        return a;
...
string Folder.shortGuiLocalPath () {
    string p = _definition.localPath;
    string home = QDir.homePath ();
    if (!home.endsWith ('/')) {
        home.append ('/');
    }
...
string Folder.cleanPath () {
    string cleanedPath = QDir.cleanPath (_canonicalLocalPath);
...
string Folder.remotePathTrailingSlash () {
...
void Folder.etagRetrieved (QByteArray &etag, QDateTime &tp) {
...
void Folder.etagRetrievedFromSyncEngine (QByteArray &etag, QDateTime &time) {
...
void Folder.createGuiLog (string &filename, LogStatus status, int count,
    const string &renameTarget) {
    if (count > 0) {
        Logger *logger = Logger.instance ();

        string file = QDir.toNativeSeparators (filename);
        string text;
...
void Folder.startVfs () {
    ...
    string stateDbFile = _journal.databaseFilePath ();
    _journal.open ();
...
int Folder.slotDiscardDownloadProgress () {
    ...
    for (auto &deleted_info : deleted_infos) {
        const string tmppath = folderpath.filePath (deleted_info._tmpfile);
...
void Folder.slotWatchedPathChanged (string &path, ChangeReason reason) {
...
void Folder.implicitlyHydrateFile (string &relativepath) {
...
void Folder.setRootPinState (PinState state) {
    if (!_vfs.setPinState (string (), state)) {
...
bool Folder.supportsSelectiveSync () {
    return !virtualFilesEnabled () && !isVfsOnOffSwitchPending ();
}

void Folder.saveToSettings () {
    // Remove first to make sure we don't get duplicates
    removeFromSettings ();

    auto settings = _accountState.settings ();
    string settingsGroup = QStringLiteral ("Multifolders");

    // True if the folder path appears in only one account
    const auto folderMap = FolderMan.instance ().map ();
    const auto oneAccountOnly = std.none_of (folderMap.cbegin (), folderMap.cend (), [this] (auto *other) {
...
void Folder.removeFromSettings () {
    auto settings = _accountState.settings ();
...
bool Folder.isFileExcludedAbsolute (string &fullPath) {
    return _engine.excludedFiles ().isExcluded (fullPath, path (), _definition.ignoreHiddenFiles);
}

bool Folder.isFileExcludedRelative (string &relativePath) {
...
void Folder.wipeForRemoval () {
    ...
    string stateDbFile = _engine.journal ().databaseFilePath ();
...
void Folder.startSync (QStringList &pathList) {
    Q_UNUSED (pathList)
...
void Folder.slotSyncError (string &message, ErrorCategory category) {
    _syncResult.appendErrorString (message);
    emit ProgressDispatcher.instance ().syncError (alias (), message, category);
}

void Folder.slotAddErrorToGui (SyncFileItem.Status status, string &errorMessage, string &subject) {
...
void Folder.slotTransmissionProgress (ProgressInfo &pi) {
...
void Folder.slotNewBigFolderDiscovered (string &newF, bool isExternal) {
    auto newFolder = newF;
    ...
    if (ok1) {
        ...
        string message = !isExternal ? (tr ("A new folder larger than %1 MB has been added : %2.\n")
...
void Folder.schedulePathForLocalDiscovery (string &relativePath) {
    _localDiscoveryTracker.addTouchedPath (relativePath.toUtf8 ());
}

void Folder.slotFolderConflicts (string &folder, QStringList &conflictPaths) {
...
void Folder.warnOnNewExcludedItem (SyncJournalFileRecord &record, QStringRef &path) {
...
void Folder.slotWatcherUnreliable (string &message) {
...
void Folder.registerFolderWatcher () {
    ...
    connect (_folderWatcher.data (), &FolderWatcher.pathChanged,
        this, [this] (string &path) { slotWatchedPathChanged (path, Folder.ChangeReason.Other); });
...
bool Folder.virtualFilesEnabled () {
...
string Folder.fileFromLocalPath (string &localPath) {
...
bool FolderDefinition.load (QSettings &settings, string &alias,
    ...
    string vfsModeString = settings.value (QStringLiteral ("virtualFilesMode")).toString ();
...
string FolderDefinition.prepareLocalPath (string &path) {
    string p = QDir.fromNativeSeparators (path);
...
string FolderDefinition.prepareTargetPath (string &path) {
    string p = path;
    if (p.endsWith (QLatin1Char ('/'))) {
        p.chop (1);
    }
...
string FolderDefinition.absoluteJournalPath () {
    return QDir (localPath).filePath (journalPath);
}

string FolderDefinition.defaultJournalPath (AccountPtr account) {
