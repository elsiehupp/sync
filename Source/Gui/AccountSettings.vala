...
protected slots:
    ...
    void slotSetSubFolderAvailability (Folder *folder, string &path, PinState state);
    ...
    void doExpand ();
    void slotLinkActivated (string &link);

    // Encryption Related Stuff.
    void slotShowMnemonic (string &mnemonic);
...
private:
    void showConnectionLabel (string &message,
        QStringList errors = QStringList ());
    bool event (QEvent *) override;
    void createAccountToolbox ();
    void openIgnoredFilesDialog (string & absFolderPath);
    void customizeStyle ();

    /// Returns the alias of the selected folder, empty string if none
    string selectedFolderAlias ();
    ...
    ***********************************************************/
    ...
    AccountSettings.AccountSettings (AccountState *accountState, Gtk.Widget *parent)
        : Gtk.Widget (parent)
        ...
        connect (_ui._folderList, &Gtk.Widget.customContextMenuRequested,
        ...
        /*QColor color = palette ().highlight ().color ();
         _ui.quotaProgressBar.setStyleSheet (string.fromLatin1 (progressBarStyleC).arg (color.name ()));*/
    ...
    void AccountSettings.slotEncryptFolderFinished (int status) {
        ...
        Q_ASSERT (folder);
        const auto path = job.property (propertyPath).value<string> ();
    ...
    string AccountSettings.selectedFolderAlias () {
    ...
    void AccountSettings.slotShowMnemonic (string &mnemonic) {
    ...
    void AccountSettings.slotEditCurrentLocalIgnoredFiles () {
        ...
        if (!selected.isValid () || _model.classify (selected) != FolderStatusModel.SubFolder)
            return;
        string fileName = _model.data (selected, FolderStatusDelegate.FolderPathRole).toString ();
        openIgnoredFilesDialog (fileName);
    }
    
    void AccountSettings.openIgnoredFilesDialog (string & absFolderPath) {
        Q_ASSERT (QFileInfo (absFolderPath).isAbsolute ());
    
        const string ignoreFile = absFolderPath + ".sync-exclude.lst";
    ...
    void AccountSettings.slotCustomContextMenuRequested (QPoint &pos) {
        ...
        string alias = _model.data (index, FolderStatusDelegate.FolderAliasRole).toString ();
        ...
        if (Theme.instance ().showVirtualFilesOption ()
            ...
            if (mode == Vfs.WindowsCfApi || ConfigFile ().showExperimentalOptions ()) {
                ac = menu.addAction (tr ("Enable virtual file support %1 …").arg (mode == Vfs.WindowsCfApi ? string () : tr (" (experimental)")));
    ...
    void AccountSettings.slotAddFolder () {
        ...
        connect (folderWizard, &Gtk.Dialog.accepted, this, &AccountSettings.slotFolderWizardAccepted);
        connect (folderWizard, &Gtk.Dialog.rejected, this, &AccountSettings.slotFolderWizardRejected);
        folderWizard.open ();
    }
    ...
    void AccountSettings.slotRemoveCurrentFolder () {
        ...
        if (selected.isValid () && folder) {
            ...
            string shortGuiLocalPath = folder.shortGuiLocalPath ();
    ...
    void AccountSettings.slotSetSubFolderAvailability (Folder *folder, string &path, PinState state) {
    ...
    void AccountSettings.showConnectionLabel (string &message, QStringList errors) {
        const string errStyle = QLatin1String ("color:#ffffff; background-color:#bb4d4d;padding:5px;"
        ...
        if (errors.isEmpty ()) {
            ...
            _ui.connectLabel.setToolTip (string ());
            _ui.connectLabel.setStyleSheet (string ());
        } else {
    ...
    void AccountSettings.slotUpdateQuota (int64 total, int64 used) {
        if (total > 0) {
            ...
            string usedStr = Utility.octetsToString (used);
            string totalStr = Utility.octetsToString (total);
            string percentStr = Utility.compactFormatDouble (percent, 1);
            string toolTip = tr ("%1 (%3%) of %2 in use. Some folders, including network mounted or shared folders, might have different limits.").arg (usedStr, totalStr, percentStr);
            ...
            _ui.quotaInfoLabel.setToolTip (string ());
    ...
    void AccountSettings.slotAccountStateChanged () {
        ...
        if (state != AccountState.Disconnected) {
            ...
            safeUrl.setPassword (string ()); // Remove the password from the URL to avoid showing it in the UI
            const auto folders = FolderMan.instance ().map ().values ();
            for (Folder *folder : folders) {
                _model.slotUpdateFolderState (folder);
            }
    
            const string server = string.fromLatin1 ("<a href=\"%1\">%2</a>")
                                       .arg (Utility.escape (account.url ().toString ()),
                                           Utility.escape (safeUrl.toString ()));
            string serverWithUser = server;
    ...
    void AccountSettings.slotLinkActivated (string &link) {
        ...
        if (li.count () > 1) {
            ...
            QModelIndex folderIndx = _model.indexForPath (f, string ());
    ...
    void AccountSettings.refreshSelectiveSyncStatus () {
        ...
        for (Folder *folder : folders) {
            ...
            for (auto &it : undecidedList) {
                ...
                if (theIndx.isValid ()) {
                    msg += string.fromLatin1 ("<a href=\"%1?folder=%2\">%1</a>")
        ...
        if (!msg.isEmpty ()) {
            ConfigFile cfg;
            string info = !cfg.confirmExternalStorage ()
    ...
    void AccountSettings.customizeStyle () {
        string msg = _ui.connectLabel.text ();
        Theme.replaceLinkColorStringBackgroundAware (msg);
        _ui.connectLabel.setText (msg);
