...
private slots:
    ...
    void slotConnectToOCUrl (string &);
    void slotAuthError ();

    void slotCreateLocalAndRemoteFolders (string &, string &);
...
namespace Occ {
    ...
    OwncloudSetupWizard.OwncloudSetupWizard (GLib.Object *parent)
        ...
        connect (_ocWizard, &Gtk.Dialog.finished, this, &GLib.Object.deleteLater);
    ...
    void OwncloudSetupWizard.runWizard (GLib.Object *obj, char *amember, Gtk.Widget *parent) {
        if (!wiz.isNull ()) {
    ...
    void OwncloudSetupWizard.startWizard () {
        ...
        string localFolder = Theme.instance ().defaultClientFolder ();
        ...
        string lf = QDir.fromNativeSeparators (localFolder);
    ...
    void OwncloudSetupWizard.slotCheckServer (string &urlString) {
    ...
    void OwncloudSetupWizard.slotConnectToOCUrl (string &url) {
    ...
    void OwncloudSetupWizard.slotAuthError () {
        ...
        if (!redirectUrl.isEmpty ()) {
            ...
            string path = redirectUrl.path ();
            static string expectedPath = "/" + _ocWizard.account ().davPath ();
    ...
    void OwncloudSetupWizard.slotCreateLocalAndRemoteFolders (string &localFolder, string &remoteFolder) {
        ...
        if (fi.exists ()) {
            ...
            string res = tr ("Creating local sync folder %1 …").arg (localFolder);
        ...
        if (nextStep) {
             ...
             *
            */
            qCInfo (lcWizard) << "Sanitize got URL path:" << string (_ocWizard.account ().url ().toString () + '/' + _ocWizard.account ().davPath () + remoteFolder);
    
            string newDavPath = _ocWizard.account ().davPath (),
            ...
            string newUrlPath = newDavPath + '/' + newRemoteFolder;
    ...
    void OwncloudSetupWizard.finalizeSetup (bool success) {
        const string localFolder = _ocWizard.property ("localFolder").toString ();
        if (success) {
    ...
    bool OwncloudSetupWizard.ensureStartFromScratch (string &localFolder) {
    ...
    void OwncloudSetupWizard.slotAssistantFinished (int result) {
        ...
        if (result == Gtk.Dialog.Rejected) {
            ...
            string localFolder = FolderDefinition.prepareLocalPath (_ocWizard.localFolder ());
    ...
    void OwncloudSetupWizard.slotSkipFolderConfiguration () {
        ...
        _ocWizard.close ();
        emit ownCloudWizardDone (Gtk.Dialog.Accepted);
    }
