...
private:
    ...
    ***********************************************************/
    struct PathTuple {
        ...
        static string pathAppend (string &base, string &name) {
            return base.isEmpty () ? name : base + QLatin1Char ('/') + name;
        }
        PathTuple addName (string &name) {
            PathTuple result;
            result._original = pathAppend (_original, name);
            auto buildString = [&] (string &other) {
...
signals:
    ...
    void ProcessDirectoryJob.process () {
        ...
        if (!_discoveryData._statedb.listFilesInPath (pathU8, [&] (SyncJournalFileRecord &rec) {
                auto name = pathU8.isEmpty () ? rec._path : string.fromUtf8 (rec._path.constData () + (pathU8.size () + 1));
    ...
    bool ProcessDirectoryJob.handleExcluded (string &path, string &localName, bool isDirectory, bool isHidden, bool isSymlink) {
    ...
    static bool computeLocalChecksum (QByteArray &header, string &path, SyncFileItemPtr &item) {
    ...
    void ProcessDirectoryJob.processFileAnalyzeRemoteInfo (
        ...
        auto renameCandidateProcessing = [&] (Occ.SyncJournalFileRecord &base) {
            ...
            string originalPathAdjusted = _discoveryData.adjustRenamedPath (originalPath, SyncFileItem.Up);
    ...
    void ProcessDirectoryJob.processFileAnalyzeLocalInfo (
        ...
        if (wasDeletedOnClient.first) {
            ...
            _pendingAsyncJobs++;
            string serverOriginalPath = _discoveryData._remoteFolder + _discoveryData.adjustRenamedPath (originalPath, SyncFileItem.Down);
    ...
    auto ProcessDirectoryJob.checkMovePermissions (RemotePermissions srcPerm, string &srcPath,
                                                   bool isDirectory)
    ...
    void ProcessDirectoryJob.addVirtualFileSuffix (string ==&str) {
        str.append (_discoveryData._syncOptions._vfs.fileSuffix ());
    }
    
    bool ProcessDirectoryJob.hasVirtualFileSuffix (string &str) {
        if (!isVfsWithSuffix ())
            return false;
        return str.endsWith (_discoveryData._syncOptions._vfs.fileSuffix ());
    }
    
    void ProcessDirectoryJob.chopVirtualFileSuffix (string &str) {
        if (!isVfsWithSuffix ())
            return;
    ...
    void ProcessDirectoryJob.startAsyncLocalQuery () {
        string localPath = _discoveryData._localDir + _currentFolder._local;
        ...
        connect (localJob, &DiscoverySingleLocalDirectoryJob.finishedFatalError, this, [this] (string &msg) {
            _discoveryData._currentlyActiveJobs--;
            _pendingAsyncJobs--;
            if (_serverJob)
                _serverJob.abort ();
        ...
        connect (localJob, &DiscoverySingleLocalDirectoryJob.finishedNonFatalError, this, [this] (string &msg) {
            _discoveryData._currentlyActiveJobs--;
            _pendingAsyncJobs--;
    ...
    bool ProcessDirectoryJob.isVfsWithSuffix () {
