...
void ProcessDirectoryJob.addVirtualFileSuffix (QString &str) {
    str.append (_discoveryData._syncOptions._vfs.fileSuffix ());
}

bool ProcessDirectoryJob.hasVirtualFileSuffix (QString &str) {
    if (!isVfsWithSuffix ())
        return false;
    return str.endsWith (_discoveryData._syncOptions._vfs.fileSuffix ());
}

void ProcessDirectoryJob.chopVirtualFileSuffix (QString &str) {
    if (!isVfsWithSuffix ())
        return;
