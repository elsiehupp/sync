...
QVariant ConfigFile.getPolicySetting (QString &setting, QVariant &defaultValue) {
...
QString ConfigFile.excludeFile (Scope scope) {
...
QString ConfigFile.defaultConnection () {
...
QVariant ConfigFile.retrieveData (QString &group, QString &key) {
    const QString con (group.isEmpty () ? defaultConnection () : group);
    QSettings settings (configFile (), QSettings.IniFormat);
...
bool ConfigFile.dataExists (QString &group, QString &key) {
    const QString con (group.isEmpty () ? defaultConnection () : group);
    QSettings settings (configFile (), QSettings.IniFormat);
...
chrono.milliseconds ConfigFile.remotePollInterval (QString &connection) {
...
chrono.milliseconds ConfigFile.forceSyncInterval (QString &connection) {
...
chrono.milliseconds ConfigFile.notificationRefreshInterval (QString &connection) {
    QString con (connection);
    if (connection.isEmpty ())
        con = defaultConnection ();
    QSettings settings (configFile (), QSettings.IniFormat);
...
chrono.milliseconds ConfigFile.updateCheckInterval (QString &connection) {
    QString con (connection);
    if (connection.isEmpty ())
        con = defaultConnection ();
    QSettings settings (configFile (), QSettings.IniFormat);
...
bool ConfigFile.skipUpdateCheck (QString &connection) {
    QString con (connection);
    if (connection.isEmpty ())
        con = defaultConnection ();
...
bool ConfigFile.autoUpdateCheck (QString &connection) {
    QString con (connection);
    if (connection.isEmpty ())
        con = defaultConnection ();
...
QString ConfigFile.proxyHostName () {
...
QString ConfigFile.proxyUser () {
    return getValue (QLatin1String (proxyUserC)).toString ();
}

QString ConfigFile.proxyPassword () {
...
QString ConfigFile.keychainProxyPasswordKey () {
...
QPair<bool, qint64> ConfigFile.newBigFolderSizeLimit () {
...
QString ConfigFile.certificatePath () {
...
QString ConfigFile.certificatePasswd () {
...
QString ConfigFile.clientVersionString () {
    QSettings settings (configFile (), QSettings.IniFormat);
