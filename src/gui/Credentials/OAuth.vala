...
namespace Occ {
        ...
        GLib.Object.connect (&_server, &QTcpServer.newConnection, this, [this] {
            while (QPointer<QTcpSocket> socket = _server.nextPendingConnection ()) {
                ...
                GLib.Object.connect (socket.data (), &QIODevice.readyRead, this, [this, socket] {
                    ...
                    string code = rxMatch.captured (1); // The 'code' is the first capture of the regexp
                    ...
                    QUrlQuery arguments (string (
                        "grant_type=authorization_code&code=%1&redirect_uri=http://localhost:%2")
                                            .arg (code, string.number (_server.serverPort ())));
                    ...
                    GLib.Object.connect (job, &SimpleNetworkJob.finishedSignal, this, [this, socket] (QNetworkReply *reply) {
                        ...
                        string user = json["user_id"].toString ();
    ...
    QUrl OAuth.authorisationLink () {
        Q_ASSERT (_server.isListening ());
        QUrlQuery query;
        query.setQueryItems ({ { QLatin1String ("response_type"), QLatin1String ("code") }, { QLatin1String ("client_id"), Theme.instance ().oauthClientId () }, { QLatin1String ("redirect_uri"), QLatin1String ("http://localhost:") + string.number (_server.serverPort ()) } });
