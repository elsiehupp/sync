...
mod a prime (mod is sooo slow!).  If you need less than 3
use a bitmask.  For exampl
  h = (h & hashmask (10));
...
@return    Returns a 32-bit value.  Every bit of the key affects ever
...
The best hash table sizes are powers of 2.  There is no need to
mod a prime (mod is sooo slow!).  If you need less than 6
use a bitmask.  For exampl
  h = (h & hashmask (10));
In which case, the hash table should have hashsize (10) elements.
