...
public class BulkPropagatorJob : AbstractPropagatorJob {
    ...
    ***********************************************************/
    public new bool on_signal_schedule_self_or_child () {
        ...
        //          file_to_upload_path = this.propagator.full_local_path (file_to_upload.file);
    ...
    ***********************************************************/
    private void on_signal_compute_transmission_checksum (
        ...
        //  );
        //  compute_checksum.release ().start (file_to_upload_path);
    }
    ...
    ***********************************************************/
    private void on_signal_start_upload (
        ...
        //  if (!FileSystem.file_exists (file_to_upload_path)) {
        //      this.pending_checksum_files.remove (item.file);
        //      on_signal_error_start_folder_unlock (item, SyncFileItem.Status.SOFT_ERROR, _("File Removed (start upload) %1").printf (file_to_upload_path));
        ...
        //  file_to_upload.size = FileSystem.get_size (file_to_upload_path);
    ...
    ***********************************************************/
    private void do_start_upload (
        ...
        //      item,
        //      file_to_upload,
        //      remote_path, file_to_upload_path,
        //      file_to_upload.size, current_headers
        //  );

        //  GLib.info (remote_path + " transmission checksum " + transmission_checksum_header + file_to_upload_path);
    ...
    ***********************************************************/
    private void adjust_last_job_timeout (
        AbstractNetworkJob abstract_netowrk_job,
        int64 file_size
    ) {
    ...
    ***********************************************************/
    private void on_signal_put_finished_one_file (
        ...
        //  var file_to_upload_path = this.propagator.full_local_path (single_file.item.file);

        //  // Check if the file still exists
        //  if (!check_file_still_exists (single_file.item, finished, file_to_upload_path)) {
        //      return;
        //  }

        //  // Check whether the file changed since discovery. the file check here is the original and not the temporary.
        //  if (!check_file_changed (single_file.item, finished, file_to_upload_path)) {
        //      return;
        //  }
    ...
    ***********************************************************/
    private bool check_file_changed (
        ...
        //  if (!FileSystem.verify_file_unchanged (file_to_upload_path, item.size, item.modtime)) {
        //      this.propagator.another_sync_needed = true;
        //      if (!finished) {
